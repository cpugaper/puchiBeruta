    ...
    if (showInspector)
    {
        ...
        if (selectedObject) {
            ...
            if (meshData) {
                ...
                ImGui::Text("Vertices: %d", meshData->vertices.size() / 3);  // Asumimos que cada v�rtice tiene 3 componentes (x, y, z)
                ImGui::Text("Indices: %d", meshData->indices.size() / 3);   // Asumimos que cada cara tiene 3 �ndices (tri�ngulos)

                // Opci�n para visualizar las normales
                bool showNormals = false;
                if (ImGui::CollapsingHeader("Show Normals")) {
                    showNormals = true;

                    // Mostrar normales por tri�ngulo
                    ...
                    // Opci�n para ver las normales por cara (media de las normales de tri�ngulos)
